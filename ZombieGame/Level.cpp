...
Level::Level(const std::string& fileName)
{
	...
	if (file.fail())
	{
		JEngine::fatalError("Failed to open " + fileName);
	}
	...
	for (size_t y = 0; y < _levelData.size(); y++)
	{
		for (size_t x = 0; x < _levelData[y].size(); x++)
		{
			...
			case 'G':
				_spriteBatch.draw(destRect,
					uvRect,
					JEngine::ResourceManager::getTexture("Textures/glass.png").id,
					0.0f,
					whiteColor);
				break;
			case 'L':
				_spriteBatch.draw(destRect,
					uvRect,
					JEngine::ResourceManager::getTexture("Textures/light_bricks.png").id,
					0.0f,
					whiteColor);
				break;
			case '@':
